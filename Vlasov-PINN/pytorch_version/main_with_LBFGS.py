...
class PhysicsInformedNN():
    def __init__(self, X, E, f, layers):
        ...
        self.optimizer_LBFGS = torch.optim.LBFGS(
            ...
            max_eval=8000,
            history_size=50,
            tolerance_grad=1e-200,
            tolerance_change=1e-400,
        )
        self.iter = 0
        self.loss_list = []
    ...
    def closure(self):
        ...
        if self.iter % 100 == 0:
            ...
            torch.save(self.dnn.state_dict(), os.path.join(save_path, "PINN_LBFGS_" + str(self.iter) + ".pth"))
        return loss

    def train(self):
        self.dnn.train()
        self.optimizer_LBFGS.step(self.closure)
        np.savetxt(os.path.join(save_path, "lbfgs_loss.txt"), self.loss_list, delimiter=" ")
        torch.save(self.dnn.state_dict(), os.path.join(save_path, "PINN_with_lbfgs.pth"))
...
model.load_model("PINN_10000.pth")
print(model.lambda_1.item(), model.lambda_2.item())
# train with LBFGS
model.train()